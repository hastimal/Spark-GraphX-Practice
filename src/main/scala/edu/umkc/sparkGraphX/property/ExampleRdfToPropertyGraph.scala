 ...
 */
  object  ExampleRdfToPropertyGraph {
    ...
    def main(args: Array[String]) {
      ...
      val source = Source.fromFile("src/main/resources/outputData/MergeRdf1Rdf2.nt", "iso-8859-1")
      ...
      for (i <- 0 until literalPropsTriplesArray.length) {
        if (literalPropsTriplesArray(i)._2 ==
          vertexURIMap("http://umkc.edu/xPropGraph#advisor")) { // if changes URL no changes even :)
     ...
     edgeRDD.foreach(println)
      ...
      val skosRelatedSubgraph =
        graph.subgraph(t => t.attr ==
          "http://umkc.edu/xPropGraph#advisor")
